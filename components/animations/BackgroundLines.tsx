...
const TABLET_BACKGROUND_QUERY = "(max-width: 1024px)";
...
const getGradientDuration = (index: number) => 10 + (index % 9);
const getGradientDelay = (index: number) => (index % 10) * 0.7;
const getGradientEndY = (index: number) => `${93 + (index % 8)}%`;

export const BackgroundLines = React.memo(
  ({ className }: { className?: string }) => {
    const prefersReducedMotion = useReducedMotion();
    const [useStaticBackground, setUseStaticBackground] = React.useState(true);
    ...
    React.useEffect(() => {
      ...
      const mediaQuery = window.matchMedia(TABLET_BACKGROUND_QUERY);
      const updateBackgroundMode = () => {
        setUseStaticBackground(mediaQuery.matches);
      };
              ...
              paths.map((_, index) => (
                <motion.linearGradient
                  ...
                  animate={{
                    ...
                    x2: ["0%", "95%"],
                    y1: ["0%", "100%"],
                    y2: ["0%", getGradientEndY(index)],
                  }}
                  transition={{
                    duration: getGradientDuration(index),
                    ease: "easeInOut",
                    repeat: Infinity,
                    delay: getGradientDelay(index),
                  }}
                >
